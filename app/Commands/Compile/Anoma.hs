...
runCommand opts@AnomaOptions {..} = do
  ...
  if
      | _anomaModular -> do
          ...
          outputAnomaModuleTable (opts ^. anomaOutputText) (copts' ^. compileDebug) nockmaFile mid mtab
      | otherwise -> do
          r <- runError @JuvixError $ runPipeline opts' (_anomaCompileCommonOptions ^. compileInputFile) upToAnoma
          res <- getRight r
          outputAnomaModule (opts ^. anomaOutputText) (copts' ^. compileDebug) nockmaFile res

outputAnomaModule :: (Members '[EmbedIO, App, Files] r) => Bool -> Bool -> Path Abs File -> Nockma.Module -> Sem r ()
outputAnomaModule textOutput debugOutput nockmaFile Nockma.Module {..} = do
  ...
  when textOutput (writeFileEnsureLn textNockmaFile (Nockma.ppSerialize code))
  when debugOutput (writeFileEnsureLn prettyNockmaFile (Nockma.ppPrint code))

outputAnomaModuleTable :: (Members '[EmbedIO, App, Files] r) => Bool -> Bool -> Path Abs File -> ModuleId -> Nockma.ModuleTable -> Sem r ()
outputAnomaModuleTable textOutput debugOutput nockmaFile mid mtab = do
  let md = Nockma.lookupModuleTable mtab mid
  outputAnomaModule textOutput debugOutput nockmaFile md
