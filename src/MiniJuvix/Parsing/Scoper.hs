...
import MiniJuvix.Utils.Prelude hiding (State, get, gets, put, modify, Reader, ask, asks, local)
import Polysemy
...
checkImport ∷ Members '[Error ScopeError, State ModuleCurrentScope, Reader ScopeParameters, Embed IO, State ModulesCache] r ⇒
  Import → Sem r (Module 'Scoped 'ModuleTop)
...
readParseModule ∷ Members '[Error ScopeError, Reader ScopeParameters, Embed IO] r ⇒
  ModulePath → Sem r (Module 'Parsed 'ModuleTop)
...
addOperatorSyntaxDef ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  OperatorSyntaxDef → Sem r ()
addOperatorSyntaxDef = undefined

checkTypeSignature ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  TypeSignature 'Parsed → Sem r (TypeSignature 'Scoped)
checkTypeSignature = undefined

checkDataTypeDef ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  DataTypeDef 'Parsed → Sem r (DataTypeDef 'Scoped)
checkDataTypeDef = undefined

checkTopModule ∷ ∀ r. Members '[Error ScopeError, State ModuleCurrentScope, Reader ScopeParameters, State ModulesCache, Embed IO] r ⇒
  Module 'Parsed 'ModuleTop → Sem r (Module 'Scoped 'ModuleTop)
checkTopModule (Module path stmts) = do
  ...
  checkedModule = Module path <$>
    local addParent (mapM checkStatement stmts)
  checkCycle ∷ Sem r ()
  checkCycle = whenM (HashSet.member path <$> asks _scopeTopParents)
               (throw (ErrImportCycle path))

checkLocalModule ∷ ∀ r. Members '[Error ScopeError, State ModuleCurrentScope, Reader ScopeParameters, State ModulesCache, Embed IO] r ⇒
  Module 'Parsed 'ModuleLocal → Sem r (Module 'Scoped 'ModuleLocal)
checkLocalModule (Module path stmts) = Module path <$> mapM checkStatement stmts

checkOpenModule ∷ ∀ r. Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  OpenModule → Sem r ()
...
openOperatorSyntaxDef ∷ ∀ r. Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  ModulePath → OperatorSyntaxDef → Sem r ()
openOperatorSyntaxDef path d = undefined

openModule ∷ ∀ r. Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  ModulePath → ModuleScopeInfo → Maybe UsingHiding → Sem r ()
...
checkFunctionClause ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  FunctionClause 'Parsed → Sem r (FunctionClause 'Scoped)
checkFunctionClause = undefined

checkAxiom ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  AxiomDef 'Parsed → Sem r (AxiomDef 'Scoped)
checkAxiom = undefined

checkEval ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  Eval 'Parsed → Sem r (Eval 'Scoped)
checkEval (Eval s) = Eval <$> checkParseExpressionSections s

checkPrint ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  Print 'Parsed → Sem r (Print 'Scoped)
checkPrint (Print s) = Print <$> checkParseExpressionSections s

checkFunction ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  Function 'Parsed → Sem r (Function 'Scoped)
checkFunction = undefined

checkLetBlock ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  LetBlock 'Parsed → Sem r (LetBlock 'Scoped)
checkLetBlock = undefined

checkLambda ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  Lambda 'Parsed → Sem r (Lambda 'Scoped)
checkLambda = undefined

checkExpressionSection ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
...
checkExpressionSections ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
...
checkParseExpressionSections ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  ExpressionSections 'Parsed → Sem r Expression
checkParseExpressionSections = checkExpressionSections >=> parseExpressionSections

checkStatement ∷ Members '[Error ScopeError, Reader ScopeParameters, Embed IO, State ModulesCache, State ModuleCurrentScope] r
...
parseExpressionSections ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
  ExpressionSections 'Scoped → Sem r Expression
parseExpressionSections = undefined

parsePatternSections ∷ Members '[Error ScopeError, State ModuleCurrentScope] r ⇒
