      ...
      WithPath
        ...
        ( a ::
          ...
          let runner ::
              ...
              helper :: (forall w. Sem r w -> Sem localEs w) -> (forall w. Sem localEs w -> Sem r w) -> Sem r x
              helper lift unlift = do
                unlift
                  ( do
                      void $ lift (ask @EntryPoint)
                      impose runner handler y
                  )

              inner :: Sem r x
              inner = localLiftUnlift localEnv SeqUnlift helper
