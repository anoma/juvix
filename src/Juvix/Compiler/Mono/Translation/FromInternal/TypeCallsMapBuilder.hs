module Juvix.Compiler.Mono.Translation.FromInternal.TypeCallsMapBuilder (buildTypeCallMap) where
...
buildTypeCallMap :: Internal.InternalTypedResult -> TypeCallsMap
buildTypeCallMap r =
  run
    ...
    $ modules
  where
    modules = r ^. Internal.resultModules
