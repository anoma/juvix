...
eval herr ctx env0 = convertRuntimeNodes . eval' env0
  where
    ...
    eval' :: Env -> Node -> Node
    eval' !env !n = trace ("evaluating: " <> ppTrace n <> "\nboop") $
      let res = case n of
            NVar (Var _ idx) -> env !! idx
            NIdt (Ident _ sym) -> eval' [] (lookupContext n sym)
            NCst {} -> n
            NApp (App i l r) -> trace "eval App " $
              case eval' env l of
            ...
            NRec (LetRec _ vs b) -> trace "eval LetRec " $
            ...
            NDyn {} -> n
            Closure {} -> n
     in trace ("finished evaluating: " <> ppTrace n <> "\nend eval" <> "\nresult= " <> ppTrace res) $ res
